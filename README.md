...
## Adding work later (no coding needed)
See `README_HOSTING_GUIDE.md` for full setup steps. Once hosted on Netlify with the admin panel enabled, work items are added/removed from `yoursite.netlify.app/admin` — no HTML editing required. New items are stored in `content/work.json` and rendered automatically by `portfolio.js`; uploaded images land in `work/uploads/`. The design and layout do not need to be rebuilt.
